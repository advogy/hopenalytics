...
- **Hashtag tracking** — admins define any number of hashtags to watch across Instagram, TikTok, and YouTube; matched posts are fetched on the same weekly schedule and summarized on the dashboard, with a "Perbandingan Hastag" comparison tab alongside the platform/metric comparisons on every Analitik & Grafik scope.
...
- **Account directory** — a searchable, filterable list of every social account (church, institution, personal, and Uni/Daerah), with a clear marker for accounts flagged "manual" (auto-fetch off). Adding an account warns on likely duplicate handles/entity names before saving.
...
- **Data export** — download PDF, Word, or Excel reports for a church, an institution, a personal account, a Uni/Daerah (organisasi) account, a single social account's history, or any comparison/leaderboard view.
