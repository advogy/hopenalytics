...
- **Automated monitoring** — data is fetched automatically every week on a configurable schedule (via Apify scrapers, or YouTube's official API), or manually at any time (single account, one Union at a time, or a bulk nationwide "Semua Data" batch — the "Dapatkan Data Terbaru" button on Analitik & Grafik now links to Monitoring Antrean to pick one of these, instead of always kicking off the slow nationwide batch directly), with a live progress bar and the ability to cancel an in-progress refresh. A newly-added account doesn't fetch immediately — it waits for the next scheduled run or a manual refresh, so typos get caught before they burn an API call. A permanently-broken account (not found, missing data) fails fast instead of retrying (retries are capped at 2 attempts otherwise); a Facebook account can't be saved with auto-fetch on unless it has a Profile URL, and one that's already missing one gets auto-fetch turned off automatically until it's filled in. Facebook accounts also sample their 10 most recent posts (count/likes/comments/shares) alongside the follower count. Batch progress correctly reaches 100% once every account has actually been attempted, even if some failed — a stuck-at-99% batch from before this fix self-heals the next time Monitoring Antrean loads.
- **Hashtag tracking** — admins define any number of hashtags to watch across every enabled platform (Instagram, TikTok, YouTube, Facebook, X, Threads); matched posts are fetched on the same weekly schedule, or on demand via a manual "Scan Ulang Sekarang" rescan (e.g. to monitor a live hashtag launch), and summarized on the dashboard, with a "Perbandingan Hastag" comparison tab (its own Uni/Daerah region filter, export, an interaction summary card totaling likes/comments/views/shares across every matching post, and a growth chart that switches from per-date to a gap-filled per-hour view for a chosen monitoring window) alongside the platform/metric comparisons on every Analitik & Grafik scope.
- **Platform & metric visibility toggles** — superadmin-only Settings cards to turn any platform, or any individual metric component (Post/Video, Views, Likes, Comment, Share, Followers/Subscribers), on/off app-wide. A disabled platform or metric disappears from every chart, list, form, and export; a disabled metric is also fully excluded from the composite Growth Score calculation itself, not just hidden from the display. Nothing is deleted, so re-enabling either instantly restores it everywhere.
...
- **Growth score** — a composite, percentage-based score (not raw follower counts) so small and large accounts are compared fairly, computed only from whichever metric components are currently enabled in Settings. See the [/about](resources/views/about.blade.php) page in-app for the full formula.
- **Performance comparison** — rank churches, institutions, personal accounts, or Uni/Daerah's own organizational accounts by platform, by metric (Post/Video, Views, Likes, Comment, Share, Followers/Subscribers — whichever are currently enabled), by composite growth score, or compare platforms against each other. Every church, institution, personal, and Uni/Daerah account has its own read-only history page reachable from any of these views; a superadmin can additionally edit or permanently delete individual historical data points from a dedicated management page per account.
...
- **Queue monitoring** (`/queue`) — four paginated tabs (Pending / Active / Completed / Failed, each remembering its own page on next/previous); a per-Union "Fetch Now" section showing account counts and true last-fetched times pulled from the accounts themselves (not just from whichever batch happened to touch them); multi-select bulk retry/delete on failed jobs; cancel a running batch or clear history individually or in bulk; plain-language error messages instead of raw stack traces.
