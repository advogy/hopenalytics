...
@section('title', __('location_import.title') . ' — ' . config('app.name'))

@section('content')
    <div class="mb-6">
        <h1 class="mb-1 text-3xl font-bold tracking-tight text-slate-900 dark:text-white">{{ __('location_import.title') }}</h1>
        <p class="text-sm text-slate-500 dark:text-slate-400">{{ __('location_import.subtitle') }}</p>
    </div>
        ...
        @foreach (['gereja' => __('common.church'), 'personal' => __('common.personal'), 'institusi' => __('common.institution')] as $type => $label)
            <div class="rounded-2xl border border-black/5 bg-white p-6 shadow-sm dark:border-white/5 dark:bg-slate-900">
                ...
                <p class="mb-4 text-sm text-slate-500 dark:text-slate-400">
                    {{ __('location_import.missing_count', ['count' => $counts[$type]]) }}
                </p>

                <a
                    href="{{ route('admin.location-import.template', $type) }}"
                ...
                >
                    ...
                    {{ __('location_import.download_template') }}
                </a>

                <form method="POST" action="{{ route('admin.location-import.import') }}" enctype="multipart/form-data" class="space-y-3">
                    @csrf
                    ...
                    >
                        ...
                        {{ __('location_import.upload_button') }}
                    </button>
                </form>
            </div>
        @endforeach
    ...
    <div class="mt-6 rounded-2xl border border-black/5 bg-white p-6 shadow-sm dark:border-white/5 dark:bg-slate-900">
        <h2 class="mb-2 text-sm font-bold text-slate-900 dark:text-white">{{ __('location_import.how_title') }}</h2>
        <ol class="list-decimal space-y-1 pl-5 text-sm text-slate-500 dark:text-slate-400">
            <li>{{ __('location_import.how_step_1') }}</li>
            <li>{{ __('location_import.how_step_2') }}</li>
            <li>{{ __('location_import.how_step_3') }}</li>
            <li>{{ __('location_import.how_step_4') }}</li>
        </ol>
    </div>
@endsection
