...
@section('content')
    @can('browse-directory-analytics')
        <a href="{{ route('churches.directory', ['tab' => 'personal']) }}" class="mb-4 inline-flex items-center gap-1 text-sm text-slate-500 hover:text-blue-600 dark:text-slate-400 dark:hover:text-blue-400">
            &larr; {{ __('nav.directory') }}
        </a>
    @endcan
    ...
    <div class="mb-8 flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
            {{-- A member editing their own account manages their name via Profil Saya instead
                 — this link is only for an admin editing someone else's Person record. --}}
            @can('update', $person)
                @if ($person->user_id !== auth()->id())
                    <a href="{{ route('people.edit', $person) }}" class="text-sm text-slate-500 hover:text-blue-600 dark:text-slate-400 dark:hover:text-blue-400">
                        {{ __('common.edit') }}
                    </a>
                @endif
            @endcan
