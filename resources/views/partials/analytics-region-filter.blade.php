...
--}}
@if ($isNasionalView)
    <div class="relative w-full sm:w-auto" data-searchable-select data-{{ $prefix }}-union>
        ...
        <input
            ...
            class="w-full sm:w-40 rounded-full border border-black/10 bg-slate-50 py-2.5 pr-9 pl-9 text-sm font-medium shadow-sm transition hover:bg-slate-100 dark:border-white/10 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700"
        >
...
@if ($isNasionalView || $isUniView)
    <div class="relative w-full sm:w-auto" data-searchable-select data-{{ $prefix }}-conference>
        <x-icon name="globe-alt" class="pointer-events-none absolute top-1/2 left-3.5 z-10 h-4 w-4 -translate-y-1/2 text-slate-400" />
