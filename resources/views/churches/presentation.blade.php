    ...
    <div class="mx-auto flex h-screen max-w-7xl flex-col px-6 py-6">
        <div class="mb-6 flex flex-wrap items-center justify-between gap-4 rounded-2xl bg-[#111827] px-6 py-4">
            ...
            <div class="text-center">
                <p class="text-xs uppercase tracking-wide text-slate-400">{{ __('presentation.total_reach_all', ['label' => $scope->labelCap()]) }}</p>
                <p class="text-4xl font-bold tabular-nums">{{ number_format($totalReach) }}</p>
            </div>

            <div class="flex items-center gap-2">
                <a href="{{ $scope->presentationGrowthUrl() }}" title="{{ __('presentation.growth_icon_title') }}" class="inline-flex h-9 w-9 items-center justify-center rounded-full text-slate-400 transition hover:bg-slate-700 hover:text-white">
                    <x-icon name="arrow-trending-up" class="h-4.5 w-4.5" />
                </a>
                <a href="{{ $scope->other()->presentationUrl() }}" title="{{ $scope->other()->labelCap() }}" class="inline-flex h-9 w-9 items-center justify-center rounded-full text-slate-400 transition hover:bg-slate-700 hover:text-white">
