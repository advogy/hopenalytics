...
@section('content')
    ...
    @php
        // The region filter deliberately stays off the "semua" platform view (pre-existing —
        // unrelated to category), but category (church-scope-only) applies on every platform
        // including "semua", so each control gets its own visibility check and the card shows
        // if either would render anything.
        $showRegionFilterOverview = $platform !== 'semua' && ($isNasionalView || $isUniView);
        $showCategoryFilterOverview = $scope->type === 'gereja';
    @endphp
    @if ($showRegionFilterOverview || $showCategoryFilterOverview)
        <x-filter-card :clear-url="($selectedConferenceId || ($isNasionalView && $selectedUnionId) || ($selectedCategory ?? null)) ? $scope->platformComparisonUrl(array_filter(['platform' => $platformParam, 'sort' => $sort === 'value' ? 'value' : null])) : null">
    ...
    <div id="metric-sections" class="grid gap-6 {{ $sections->count() > 1 ? 'lg:grid-cols-2' : '' }}">
        @foreach ($sections as $metric => $rows)
            ...
            <div class="rounded-2xl border border-black/5 bg-white p-5 shadow-sm dark:border-white/5 dark:bg-slate-900">
                ...
                @if ($top->isEmpty())
                    <x-empty-state variant="inline">{{ __('comparison.no_data') }}</x-empty-state>
                @else
